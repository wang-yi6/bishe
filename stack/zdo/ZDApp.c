...
//#define  HOLD_AUTO_START
  ...
  devStates_t devState = DEV_HOLD;//不进行网络初始化
#else
  devStates_t devState = DEV_INIT;//进行初始化
#endif
...
void ZDApp_Init( uint8 task_id )
{
  ...
  {
   ZDOInitDevice( 0 );//这个地方实现组网的功能
  }
  else
  {
    ZDOInitDevice( ZDO_INIT_HOLD_NWK_START );
    // Blink LED to indicate HOLD_START
    HalLedBlink ( HAL_LED_4, 0, 50, 500 );//网络故障，不加网络
  }
...
void ZDAppCheckForHoldKey( void )
{
  ...
  if ( HalKeyRead () == SW_BYPASS_START)//如果有按键参加了，就不需要
  {
    // Change the device state to HOLD on start up
    devState = DEV_HOLD;//保存为有效
  }
#endif // HAL_KEY
}
